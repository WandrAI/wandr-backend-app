...
class TestUserProfiles:
    ...
    async def test_get_user_profile_by_id(
        self, authenticated_client: AsyncClient, second_test_user: User
    ):
        """Test retrieving another user's public profile."""
        response = await authenticated_client.get(
            f"/api/v1/users/{second_test_user.id}"
        )
    ...
    async def test_get_user_profile_unauthenticated(
        self, test_client: AsyncClient, second_test_user: User
    ):
    ...
    async def test_update_own_profile_via_correct_endpoint(
        ...
        response = await authenticated_client.put(
            "/api/v1/users/me/profile", json=update_data
        )
    ...
    async def test_update_other_user_profile_endpoint_does_not_exist(
        self, authenticated_client: AsyncClient, second_test_user: User
    ):
        ...
        response = await authenticated_client.put(
            f"/api/v1/users/{second_test_user.id}/profile", json=update_data
        )
...
class TestUserPrivacy:
    ...
    async def test_user_email_privacy(
        self, authenticated_client: AsyncClient, second_test_user: User
    ):
    ...
    async def test_user_password_privacy(
        self, authenticated_client: AsyncClient, second_test_user: User
    ):
    ...
    async def test_inactive_user_not_visible(
...
class TestUserValidation:
    ...
    async def test_profile_update_validation(
        ...
        for invalid_data in invalid_updates:
            response = await authenticated_client.put(
                "/api/v1/users/me/profile", json=invalid_data
            )
