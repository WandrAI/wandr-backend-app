...
class TestTripCRUD:
    ...
    async def test_create_trip_success(self, authenticated_client: AsyncClient, test_user: User, db_session: AsyncSession):
        ...
        assert await DatabaseTestUtils.verify_trip_exists(db_session, trip_data["title"])
    ...
    @pytest.mark.parametrize("invalid_data", [
        {"title": "", "description": "desc", "start_date": "2024-07-01", "end_date": "2024-07-15"},  # Empty title
        {"title": "Trip", "description": "desc", "start_date": "invalid-date", "end_date": "2024-07-15"},  # Invalid date
        {"title": "Trip", "description": "desc", "start_date": "2024-07-15", "end_date": "2024-07-01"},  # End before start
        {},  # Missing required fields
    ])
    async def test_create_trip_invalid_data(self, authenticated_client: AsyncClient, invalid_data):
    ...
    async def test_get_user_trips(self, authenticated_client: AsyncClient, test_user: User, db_session: AsyncSession):
        """Test retrieving user's trips."""
        # Create multiple trips
        trip_data = [
            {"title": "Trip 1", "description": "First trip", "start_date": "2024-07-01", "end_date": "2024-07-15"},
            {"title": "Trip 2", "description": "Second trip", "start_date": "2024-08-01", "end_date": "2024-08-15"},
        ]
    ...
    async def test_get_trip_by_id(self, authenticated_client: AsyncClient, test_user: User):
    ...
    async def test_update_trip_success(self, authenticated_client: AsyncClient, test_user: User):
        ...
        response = await authenticated_client.put(f"/api/v1/trips/{trip_id}", json=update_data)
    ...
    async def test_update_trip_unauthorized(self, test_client: AsyncClient, test_user: User, second_test_user: User, db_session: AsyncSession):
        ...
        response = await test_client.put(f"/api/v1/trips/{trip.id}", json=update_data, headers=headers)
        
        assert response.status_code == 403
        data = response.json()
        assert "detail" in data
    
    async def test_delete_trip_success(self, authenticated_client: AsyncClient, test_user: User):
...
class TestTripMembers:
    ...
    async def test_add_trip_member(self, authenticated_client: AsyncClient, test_user: User, second_test_user: User):
        ...
        response = await authenticated_client.post(f"/api/v1/trips/{trip_id}/members", json=member_data)
    ...
    async def test_get_trip_members(self, authenticated_client: AsyncClient, test_user: User, second_test_user: User):
        ...
        # Add member
        member_data = {"user_id": second_test_user.id, "role": "member"}
        await authenticated_client.post(f"/api/v1/trips/{trip_id}/members", json=member_data)
        ...
        added_member = next((m for m in data if m["user_id"] == second_test_user.id), None)
        assert added_member is not None
        assert added_member["role"] == "member"
    
    async def test_remove_trip_member(self, authenticated_client: AsyncClient, test_user: User, second_test_user: User):
        ...
        # Add member
        member_data = {"user_id": second_test_user.id, "role": "member"}
        add_response = await authenticated_client.post(f"/api/v1/trips/{trip_id}/members", json=member_data)
        member = add_response.json()
        member_id = member["id"]
        
        # Remove member
        response = await authenticated_client.delete(f"/api/v1/trips/{trip_id}/members/{second_test_user.id}")
        
        assert response.status_code == 200
        
        # Verify member is removed
        get_response = await authenticated_client.get(f"/api/v1/trips/{trip_id}/members")
        members = get_response.json()
        removed_member = next((m for m in members if m["user_id"] == second_test_user.id), None)
