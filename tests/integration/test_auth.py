...
class TestUserRegistration:
    ...
    async def test_register_user_success(self, test_client: AsyncClient, db_session: AsyncSession):
        ...
        assert await DatabaseTestUtils.verify_user_exists(db_session, registration_data["email"])
    
    async def test_register_user_duplicate_email(self, test_client: AsyncClient, test_user: User):
    ...
    async def test_register_user_duplicate_username(self, test_client: AsyncClient, test_user: User):
    ...
    @pytest.mark.parametrize("invalid_data", [
        ...
        {"email": "test@example.com", "username": "user", "password": "123"},  # Too short
        {"email": "", "username": "user", "password": "password123"},
        {},  # Missing fields
    ])
    async def test_register_user_invalid_data(self, test_client: AsyncClient, invalid_data):
...
class TestUserProfile:
    ...
    async def test_get_profile_authenticated(self, authenticated_client: AsyncClient, test_user: User):
    ...
    async def test_update_profile_endpoint_does_not_exist(self, authenticated_client: AsyncClient, test_user: User):
